...
public class Week1 {
    ...
    private static MediaPlayer voiceBF;
    private static MediaPlayer voiceOpponent;
    private static int frameCounter = 0;
    private static double noteSpeed;
    ...
    public static Scene getCreatedScene(boolean freePlay, int songNum) {
        //Frame rate tracker
        long[] totalLs = new long[180];

        //Which song in the week
        switch (songNum){
            case 0:
                noteSpeed = 1.3;
                BFChart = Charts.getBopeeboBFChart();
                OpponentChart = Charts.getBopeeboDadChart();
                song = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Bopeebo/Inst.mp3"));
                voiceBF = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Bopeebo/Voices-bf.mp3"));
                voiceOpponent = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Bopeebo/Voices-dad.mp3"));
                break;
            case 1:
                noteSpeed = 1.8;
                BFChart = Charts.getFreshBFChart();
                OpponentChart = Charts.getFreshDadChart();
                song = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Fresh/Inst.mp3"));
                voiceBF = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Fresh/Voices-bf.mp3"));
                voiceOpponent = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Fresh/Voices-dad.mp3"));
                break;
            case 2:
                noteSpeed = 2.3;
                BFChart = Charts.getDadbattleBFChart();
                OpponentChart = Charts.getDadbattleDadChart();
                song = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Dadbattle/Inst.mp3"));
                voiceBF = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Dadbattle/Voices-bf.mp3"));
                voiceOpponent = new MediaPlayer(new Media(Main.class.getResource("") + "Sounds/Music/Dadbattle/Voices-dad.mp3"));
                break;
            default:
        }
        ...
        //Camera
        int[] cameraPos = {700, 419};

        //Characters
        BFAnimated BF = new BFAnimated(0);
        GFAnimated GF = new GFAnimated(0);
        DaddyDearestAnimated Opponent = new DaddyDearestAnimated(0);
        ...
        ImageView iconOpponent = new ImageView(new Image(Main.class.getResource("") + "Images/Icons/icon-dad.png"));
        iconBF.setScaleX(-1);
        iconBF.setY(545);
        ...
        infoText.setFont(Main.getBaeFont(24));
        ...
        pauseMenu[0].setFont(Main.getBaeFont(100));
        pauseMenu[1] = new Label("Restart Song");
        pauseMenu[1].setFont(Main.getBaeFont(100));
        pauseMenu[2] = new Label("Exit to Menu");
        pauseMenu[2].setFont(Main.getBaeFont(100));
        ...
        root.getChildren().add(GF.getView());
        ...
        for (int i = 0; i < 4; i++){
            for (int e = 0; e < BFChart[i].length; e++){
                if (!BFChart[i][e].getHoldTime().equals(Duration.ZERO)){
                    ...
                    BFChart[i][e].getHold().setVisible(true);
                }
            ...
            for (int e = 0; e < OpponentChart[i].length; e++){
                ...
                OpponentChart[i][e].getView().setFitWidth(OpponentChart[i][e].getView().getViewport().getWidth() * 2d / 3d);
        ...
        AnimationTimer timer = new AnimationTimer() {
            ...
            public void handle(long l) {
                ...
                GF.nextFrame(false, false, false, false, frameCounter);
                ...
                for (int i = 0; i < 4; i++){
                    for (int e = 0; e < BFChart[i].length; e++){
                        ...
                        //Check whether the note needs to be visible
                        if (BFChart[i][e].getView().getY() > 720) {
                            BFChart[i][e].getView().setVisible(false);
                        } else if (BFChart[i][e].getView().getY() > 140 * noteSpeed / 2d + 50 + 10){
                            BFChart[i][e].getView().setVisible(true);
                        }
                        //Hold Notes
                        if (!BFChart[i][e].getHoldTime().equals(Duration.ZERO)){
                            //Check whether the hold note needs to be visible
                            if (BFChart[i][e].getView().getY() > 720) {
                        ...
                        if (BFChart[i][e].getStatus() == 3 && BFChart[i][e].getTimestamp().toMillis() + BFChart[i][e].getHoldTime().toMillis() - song.getCurrentTime().toMillis() < 0){
                            ...
                            root.getChildren().add(judgement.get(judgement.size()- 1).getImageView());
                            judgement.get(judgement.size()- 1).getImageView().setFitWidth(judgement.get(judgement.size()- 1).getImage().getWidth() * 2d / 3d);
                            judgement.get(judgement.size()- 1).getImageView().setX(700);
                            judgement.get(judgement.size()- 1).getImageView().setY(300);
                            judgement.get(judgement.size()- 1).getImageView().setPreserveRatio(true);
                            judgement.get(judgement.size()- 1).getImageView().setCache(true);
                            judgement.get(judgement.size()- 1).getImageView().setSmooth(true);
                        }
                        //Miss
                ...
                for (int i = 0; i < 4; i++){
                    for (int e = 0; e < OpponentChart[i].length; e++){
                        ...
                        if (OpponentChart[i][e].getView().getY() > 720) {
                            OpponentChart[i][e].getView().setVisible(false);
                        } else if (OpponentChart[i][e].getView().getY() > 50 + 10){
                            OpponentChart[i][e].getView().setVisible(true);
                        }
                        if (!OpponentChart[i][e].getHoldTime().equals(Duration.ZERO)){
                            ...
                            if (OpponentChart[i][e].getView().getY() > 720) {
                                ...
                                OpponentChart[i][e].getHold().setVisible(true);
                            }
                        }
                ...
                GF.getView().setX(1700 - cameraPos[0] - GF.getFrameWidth() - GF.getXOffset());
                GF.getView().setY(989 - cameraPos[1] - GF.getFrameHeight() - GF.getYOffset());
                Opponent.getView().setX(1250 - cameraPos[0] - Opponent.getFrameWidth() - Opponent.getXOffset());
                Opponent.getView().setY(1090 - cameraPos[1] - Opponent.getFrameHeight() - Opponent.getYOffset());
        ...
        song.setOnEndOfMedia(() -> {
            ...
            try {
                Main.changeScore(1 + songNum, score);
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
            if (!freePlay) {
                switch (songNum){
                    case 0:
                        Main.getStage().setScene(getCreatedScene(false, 1));
                        break;
                    case 1:
                        Main.getStage().setScene(getCreatedScene(false, 2));
                        break;
                    case 2:
        ...
        retrySound.setOnEndOfMedia(() -> {
            timer.stop();
            Main.getStage().setScene(getCreatedScene(freePlay, songNum));
        });
